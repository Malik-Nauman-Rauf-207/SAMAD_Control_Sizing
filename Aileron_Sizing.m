...
S_h = 0.23;                       %m^2, Horizontal Tail Area
S_v = 0.06;%0.089;
...
Del_a = 20/57.3;                    %rad, Aileron Deflection Angle (MAX)          
Cl = Cl_dela*Del_a;                 % Roll Moment Coefficient
L_Del = 0.5*rau*(Vapp^2)*S_w*Cl*b_w;   %Nm, Rolling Moment due to Aileron
Cl_P = -0.559;                         % Rolling Drag Coefficient
L_P = Cl_P*(b_w/(2*Vapp))*((0.5*rau*(Vapp^2)*S_w*b_w)/(Ixx));
Pss = (-L_Del/L_P)*Del_a;
PSS = Pss*57.3;
